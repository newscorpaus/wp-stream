...
class Admin {
	 ...
	 * @var string
	 */
	public $settings_cap = 'stream_settings';
	...
	public function __construct( $plugin ) {
		...
		if ( is_multisite() && is_plugin_active_for_network( $this->plugin->name ) && ! is_network_admin() ) {
	...
	public function admin_enqueue_scripts( $hook ) {
		wp_register_script( 'select2', $this->plugin->dir_url . '/assets/lib/select2/select2.js', array( 'jquery' ), '3.5.2', true );
		wp_register_style( 'select2', $this->plugin->dir_url . '/assets/lib/select2/select2.css', array(), '3.5.2' );
		wp_register_script( 'timeago', $this->plugin->dir_url . '/assets/lib/timeago/jquery.timeago.js', array(), '1.4.1', true );

		$locale    = strtolower( substr( get_locale(), 0, 2 ) );
		$file_tmpl = 'assets/lib/timeago/locales/jquery.timeago.%s.js';

		if ( file_exists( $this->plugin->dir_url . sprintf( $file_tmpl, $locale ) ) ) {
			wp_register_script( 'timeago-locale', $this->plugin->dir_url . sprintf( $file_tmpl, $locale ), array( 'timeago' ), '1' );
		} else {
			wp_register_script( 'timeago-locale', $this->plugin->dir_url . sprintf( $file_tmpl, 'en' ), array( 'timeago' ), '1' );
		}

		wp_enqueue_style( 'wp-stream-admin', $this->plugin->dir_url . '/assets/css/admin.css', array(), $this->plugin->get_version() );
		...
		if ( in_array( $hook, $this->screen_id ) || in_array( $hook, $script_screens ) ) {
			...
			wp_enqueue_script( 'wp-stream-admin', $this->plugin->dir_url . '/assets/js/admin.js', array( 'jquery', 'select2' ), $this->plugin->get_version() );
			wp_enqueue_script( 'wp-stream-live-updates', $this->plugin->dir_url . '/assets/js/live-updates.js', array( 'jquery', 'heartbeat' ), $this->plugin->get_version() );
		...
		wp_enqueue_script( 'wp-stream-global', $this->plugin->dir_url . '/assets/js/global.js', array( 'jquery' ), $this->plugin->get_version() );
		wp_localize_script(
			'wp-stream-global',
			'wp_stream_global',
			array(
	...
	public function admin_menu_css() {
		wp_register_style( 'wp-stream-datepicker', $this->plugin->dir_url . '/assets/css/datepicker.css', array(), $this->plugin->get_version() );
		wp_register_style( 'wp-stream-icons', $this->plugin->dir_url . '/assets/stream-icons/style.css', array(), $this->plugin->get_version() );
		...
		$stream_url   = $this->plugin->dir_url;
		...
		if ( version_compare( $wp_version, '3.8-alpha', '>=' ) ) {
			...
			$css = "
				#toplevel_page_{$records_page} .wp-menu-image {
					background: url( {$stream_url}assets/stream-icons/menuicon-sprite.png ) 0 90% no-repeat;
				}
				...
				@media  only screen and (-moz-min-device-pixel-ratio: 1.5),
					...
					#toplevel_page_{$records_page} .wp-menu-image {
						background: url( {$stream_url}assets/stream-icons/menuicon-sprite-2x.png ) 0 90% no-repeat;
	...
	public function plugin_action_links( $links, $file ) {
		if ( plugin_basename( $this->plugin->dir_path . '/stream.php' ) !== $file ) {
			return $links;
		}

		// Also don't show links in Network Admin if Stream isn't network enabled
		if ( is_network_admin() && is_multisite() && ! is_plugin_active_for_network( $this->plugin->name ) ) {
			return $links;
		}
	...
	public function render_settings_page() {
		...
		wp_enqueue_script( 'wp-stream-settings', $this->plugin->dir_url . '/assets/js/settings.js', array( 'jquery' ), $this->plugin->get_version(), true );
		?>
		<div class="wrap">
			<h1><?php echo esc_html( get_admin_page_title() ) ?></h1>
	...
	public function get_users_record_meta( $users ) {
		$authors_records = array();

		foreach ( $users as $user ) {
			$author = new Author( $user->ID );

			$authors_records[ $user->ID ] = array(
