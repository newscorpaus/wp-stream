...
class DB {
	...
	public function insert( $recordarr ) {
		...
		$fields = array( 'object_id', 'site_id', 'blog_id', 'user_id', 'user_role', 'created', 'summary', 'connector', 'context', 'action' );
		...
		if ( ! $record_id ) {
			 ...
			 * @param int $record_id
			 */
			do_action( 'wp_stream_record_insert_error', $recordarr, $record_id );
	...
	public function existing_records( $column ) {
		// Sanitize column
		$allowed_columns = array( 'ID', 'site_id', 'blog_id', 'object_id', 'user_id', 'user_role', 'created', 'summary', 'connector', 'context', 'action' );
