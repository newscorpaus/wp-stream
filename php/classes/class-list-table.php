...
class List_Table extends \WP_List_Table {
	...
	function assemble_records( $column ) {
		...
		foreach ( $all_records as $record => $label ) {
			if ( array_key_exists( $record, $existing_records ) ) {
				...
				//we are not disabling users in the list as retrieve logic is different for sumo
				$disabled_records[ $record ] = array( 'label' => $label, 'disabled' => '' );
			}
		}
	...
	function filter_search() {
		...
		if ( isset( $_GET['search'] ) ) {
			...
			$search = esc_attr( wp_unslash( $_GET['search'] ) );
		}
		// @codingStandardsIgnoreEnd
		$out = sprintf(
			'<p class="search-box">
